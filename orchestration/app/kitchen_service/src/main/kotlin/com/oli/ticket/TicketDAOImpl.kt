...
class TicketDAOImpl : TicketDAO {
    private fun resultRowToTicket(resultRow: ResultRow, items: List<OrderItemWithTicketId>): Ticket {
        return Ticket(
    ...
    private fun resultRowToMenuItem(resultRow: ResultRow): OrderItemWithTicketId = OrderItemWithTicketId(
    ...
    private fun readQuery(ticketId: Int): Ticket? {
        val items: List<OrderItemWithTicketId> = OrderItems.select(OrderItems.ticketId eq ticketId).toList().map(::resultRowToMenuItem)
